...
export function HmCloneHeroCaptureForm({ onSubmitLead }) {
  ...
  if (submitOk) {
    return (
      <div className="mt-7 rounded-[10px] border border-[color:var(--hm-hero-success-border)] bg-[var(--hm-hero-input-bg)] px-4 py-5 text-center text-sm leading-relaxed text-[var(--hm-hero-text)] sm:mt-8">
        Recebemos seu contato! Em breve você recebe mais informações no e-mail e no WhatsApp.
      </div>
    )
  }
  ...
  return (
    <form className="mt-7 space-y-3 sm:mt-8 sm:space-y-3.5" onSubmit={handleSubmit(onValid)} noValidate>
      ...
      <button
        type="submit"
        className="hm-hero-btn-cta flex w-full min-h-[3.25rem] cursor-pointer items-center justify-center rounded-[10px] px-7 py-3 text-center text-lg font-semibold leading-none tracking-tight text-white sm:min-h-[3.5rem] sm:px-10 sm:text-[22px]"
      >
